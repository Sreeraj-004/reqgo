...
@app.get("/colleges/search", response_model=List[schemas.CollegeOut])
def search_colleges(
    name: str,
    db: Session = Depends(get_db),
):
    ...
    """
    colleges = db.query(models.College).filter(models.College.name.ilike(f"%{name}%")).all()
    return colleges
...
def update_leave_status(
    ...
    # Enforce Department check for Student -> HOD approval
    if requester.role == "student" and approver.role == "hod":
        student_access = (
            db.query(models.AccessRequest)
            .filter(
                models.AccessRequest.user_id == requester.id,
        ...
        hod_access = (
            db.query(models.AccessRequest)
            .filter(
                models.AccessRequest.user_id == approver.id,
                models.AccessRequest.status == "approved",
            )
            .first()
        )
        ...
        if not student_access:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Student is not assigned to any department.",
            )
        if not hod_access:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        if (
            student_access.college_name != hod_access.college_name
            or student_access.department_name != hod_access.department_name
        ):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="HOD can only approve students from the same college and department.",
            )
